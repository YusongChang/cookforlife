...
window.onload = function(){
  ...
  if(send != undefined){//新增文章頁面
    ...
    url="focusArticleEdit.php?act=add";
    enter = 'send';
  }
  if(save != undefined){//更新文章頁面
    ...
    id = param.get('id');
    enter='save';
    url="focusArticleEdit.php?act=update&id="+id;
    //添加刪除按鈕點擊事件
...
function EventListener(){
  ...
  document.getElementById('thumb-img').addEventListener('click',showCoverModal);
}
...
function showCoverModal(){
  var add = document.getElementById('addThumb-btn'),
      ...
      input = document.getElementById('inputCover'), 
      save = document.getElementById('save-thumb');

  thumbModal.style.display = "block";

  save.addEventListener('click',uploadCover);//上傳封面圖片
  add.onclick = function(){ //按下canvas 觸發 input:file 的 click event
    input.addEventListener('change',previewCover);
    input.click();
  };
...
function previewCover(e){
  ...
  console.log(canvas);
   reader.onloadend = function(){
    ...
    img.onload =function(){
      // 圖片最大尺寸不能超過 72*72
      if(this.naturalWidth <= 360) canvas.width = this.naturalWidth;
      else canvas.width = 360;//設置 canvas 寬尺寸
      if(this.naturalHeight <= 270) canvas.height = this.naturalHeight;
      else canvas.height = 270;//設置 canvas 高尺寸
      getOrientation(e.target.files[0],img,canvas);
    }
  }
  reader.readAsDataURL(e.target.files[0]);
...
function uploadCover(){
  ...
  sendRequest(url,request,uploadCoverResult);
}
 ...
 * @param {Element Object} element
*/
function uploadCoverResult(xhttp){
  if( xhttp.responseText != false ){ //回傳 食譜 id 整數型別   
      window.onbeforeunload = null; //移除 onbeforeunload 事件-- 
...
function ajaxResult(xhttp){
  if( xhttp.responseText == 'SUCCESS' ){ //回傳 食譜 id 整數型別
    window.onbeforeunload = null; //移除 onbeforeunload 事件
    window.location.href="focusArticle.php?act=show";
  }else if('ERROR'){
